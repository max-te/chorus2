...
this.Kodi.module("Entities", function(Entities, App, Backbone, Marionette, $, _) {
  ...
  var API = {
    ...
    getSavedModelFromSource(item, position) {
      const newItem = {};
      for (var fieldName of Array.from(this.savedFields)) {
        if (item[fieldName]) {
  ...
  Cls = (Entities.localPlaylistItem = class localPlaylistItem extends Entities.Model {
    ...
    defaults() {
      const fields = {};
      for (var f of Array.from(API.savedFields)) {
        fields[f] = '';
      }
      return fields;
    }
  ...
  */
  ...
  App.reqres.setHandler("localplaylist:add:entity", function(name, media, type) {
    if (type == null) { type = 'list'; }
    return API.addList({name, media, type});
});
