  ...
  */
  ...
  var API = {
    ...
    getSources() {
      ...
      for (source of Array.from(this.sources)) {
      ...
      commander.multipleCommands(commands, resp => {
        for (var i in resp) {
          ...
          if (item[responseKey]) {
            for (var model of Array.from(item[responseKey])) {
    ...
    createPathCollection(file, sourcesCollection) {
      ...
      for (var source of Array.from(allSources)) {
      ...
      if (parentSource.file) {
        ...
        for (var part of Array.from(pathParts)) {
          if (part !== '') {
