...
//    model
  ...
  */
  ...
  setModelDefaultFields(defaultFields) {
    if (defaultFields == null) { defaultFields = {}; }
    defaultFields = _.extend(this.modelDefaults, defaultFields);
    return Array.from(this.getEntityFields('full')).map((field) =>
      (this.defaults[field] = ''));
  },
  ...
  */
  ...
  buildRpcRequest(type) {
    if (type == null) { type = 'read'; }
    const req = [this.callMethodName];
    for (var arg of Array.from(this.callArgs)) {
