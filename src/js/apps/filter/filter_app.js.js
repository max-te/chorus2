...
this.Kodi.module("FilterApp", function(FilterApp, App, Backbone, Marionette, $, _) {
  ...
  var API = {
    ...
    */
    ...
    setStoreFilters(filters, type) {
      if (filters == null) { filters = {}; }
      if (type == null) { type = 'filters'; }
      const store = {};
    ...
    */
    ...
    getFilterOptions(key, collection) {
      ...
      if ((s.filterCallback === 'multiple') && (s.type === 'object')) {
        ...
        const limited = [];
        for (var item of Array.from(collectionItems)) {
          for (var i in item) {
            var data = item[i];
            if (i < 5) {
    ...
    */
    ...
    getFilterSettings(key, availableOnly) {
  ...
  */
  ...
  App.reqres.setHandler('filter:init', function(availableFilters) {
    ...
    if (!_.isEmpty(params)) {
      ...
      return (() => {
        const result = [];
        for (var key of Array.from(availableFilters.filter)) {
  ...
  App.reqres.setHandler('filter:store:key:toggle', function(key, value) {
    ...
    if (_.indexOf(values, value) > -1) {
      const newValues = [];
      for (var i of Array.from(values)) {
        if (i !== value) {
          newValues.push(i);
        }
      }
