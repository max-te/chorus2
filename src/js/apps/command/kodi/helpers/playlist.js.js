...
(function() {
  const Cls = (Api.PlayList = class PlayList extends Api.Player {
    ...
    play(type, value, model, resume, callback) {
    ...
    addCollection(collection, position, callback) {
    ...
    insert(type, value, position, callback) {
      if (position == null) { position = 0; }
    ...
    insertAndPlay(type, value, position, resume, callback) {
      if (position == null) { position = 0; }
      if (resume == null) { resume = 0; }
